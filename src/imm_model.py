...
class Encoder(nn.Module):
    def __init__(self, n_landmarks=10):
        ...
        # The input to the linear layer depends on the final feature map size
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        # Output is in [-1, 1] range, suitable for scaling
...
class Generator(nn.Module):
    def __init__(self, n_landmarks=10):
        ...
        # Input: image features (512) + landmark heatmap (1) = 513 channels
        self.main = nn.Sequential(
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(256, 512, 4, 2, 1), nn.InstanceNorm2d(512), nn.LeakyReLU(0.2, inplace=_True),
